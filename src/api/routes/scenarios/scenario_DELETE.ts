...
export default function buildRoute(router: Router) {
    ...
    router.delete('/projects/:projectId/user-journeys/:journeyId/scenarios/:scenarioId', async (req: Request, resp: Response) => {
        ...
        try {
            ...
            const results = await scenarioStorage.deleteById(projectId, userJourneyId, scenarioId, storageSession);
            if (results === undefined) {
                return resp.status(500).json('Failed to delete scenario');
            }
            ...
            return resp
                .status(500)
                .json(err.message || 'Failed to delete scenario');
        } finally {
            storageSession?.ends();
        }
    })
